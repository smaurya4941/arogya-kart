...
>
    <div class="flex items-center justify-between border-b border-white/10 px-6 py-5">
        <div>
            <a href="{{ url('/') }}" class="text-2xl font-bold tracking-tight">ArogyaKart</a>
    ...
    <div class="border-b border-white/10 px-6 py-4">
        ...
        <p class="mt-1 text-xs uppercase tracking-[0.25em] text-emerald-100/75">{{ $user?->role instanceof \BackedEnum ? $user->role->value : ($user?->role ?? 'guest') }}</p>
    </div>
                ...
                @foreach ($group['items'] as $item)
                    ...
                    @if ($isAvailable)
                        <a
                            ...
                            @click="sidebarOpen = false"
                            class="{{ $baseClasses }} {{ $isActive ? 'bg-white text-primary shadow-lg shadow-emerald-950/15' : 'text-white/90 hover:bg-white/10 hover:text-white' }}"
                        >
                    ...
                    @else
                        <div class="{{ $baseClasses }} cursor-not-allowed text-white/45">
                            <span>{{ $item['label'] }}</span>
